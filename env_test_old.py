...
# IK mode constants
CARTESIAN_DELTA_SCALE = 0.02  # Meters per step for end-effector position
KUKA_IK_ACTION_DIM = 4  # 3 xyz + 1 gripper
...
class KukaPickPlaceEnv(gym.Env):
    ...
    def __init__(
        ...
        if self.use_ik:
            # IK mode: 3D end-effector delta + 1 gripper action
            action_dim = KUKA_IK_ACTION_DIM
        else:
            # Joint-space mode: 4 arm joint deltas + 1 gripper action
            action_dim = KUKA_ACTION_DIM
        ...
        #               relative_target, ee_pos, prev_action]
        # sin(q): 4, cos(q): 4, qdot: 4, grip_pos: 2, grip_vel: 2,
        # relative_target: 3, ee_pos: 3, prev_action: action_dim
        obs_dim = 4 + 4 + 4 + 2 + 2 + 3 + 3 + action_dim
    ...
    def _setup_pybullet_kuka(self):
        ...
        # Cache all movable joint indices for IK seeding (excludes fixed joints).
    ...
    def _apply_action(self, action: np.ndarray):
        """
        Map the action into target joint positions.

        In joint-space mode (use_ik=False):
        - First 4 elements: arm joint position deltas
        - 5th element: gripper action (positive = open, negative = close)

        In IK mode (use_ik=True):
        - First 3 elements: end-effector position deltas [dx, dy, dz]
        - 4th element: gripper action (positive = open, negative = close)
        """
        action = np.clip(action, self.action_space.low, self.action_space.high)

        if self.use_ik:
            # IK mode: convert Cartesian deltas to joint targets
            ee_delta = action[:3] * CARTESIAN_DELTA_SCALE
            target_ee_pos = self.last_end_effector_pos + ee_delta
            target_arm_angles = self._compute_ik(target_ee_pos)
            gripper_action = action[3] * KUKA_GRIPPER_DELTA_SCALE
        else:
            # Joint-space mode: apply deltas to current positions
        ...
        # Gripper control: positive action = open, negative = close
        gripper_states = p.getJointStates(self.robot_id, self.gripper_joint_indices)
