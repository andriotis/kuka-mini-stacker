...
def make_env(render_mode: str = None, use_ik: bool = False) -> Callable:
    """Create a factory function for the KUKA environment."""

    def _init() -> KukaPickPlaceEnv:
        return KukaPickPlaceEnv(render_mode=render_mode, use_ik=use_ik)

    return _init


def create_log_dir(base_dir: str = "./logs", use_ik: bool = False) -> dict:
    """Create timestamped log directories."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    mode_suffix = "ik" if use_ik else "joint"
    run_dir = os.path.join(base_dir, f"ppo_kuka_{mode_suffix}_{timestamp}")
...
def train(args: argparse.Namespace) -> None:
    ...
    mode_str = "IK (Cartesian)" if args.use_ik else "Joint-space"
    print(f"Control Mode: {mode_str}")
    print("=" * 60)

    # Create log directories
    log_dirs = create_log_dir(args.log_dir, use_ik=args.use_ik)
    ...
    train_env = make_vec_env(
        make_env(render_mode="rgb_array", use_ik=args.use_ik),
        n_envs=args.n_envs,
        seed=args.seed,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--use-ik",
        action="store_true",
        help="Use IK-based Cartesian control instead of joint-space control",
    )
